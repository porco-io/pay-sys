...
export const orderProcessingWorkflow = async (orderSn: string) => {
  ...
  try {
    const {
      success,
      shouldClose,
    } = await executeChild(waitPaymentWorkflow, {
      args: [orderSn],
      workflowId: `waitPayment-${orderSn}`,
    });
    // 支付失败或超时或订单被关闭
    if (!success) {
      if (shouldClose) {
        await executeChild(closeOrderWorkflow, { args: [orderSn], workflowId: `closeOrder-${orderSn}` });
      }
      return;
    }
  } catch (e) {
  ...
  switch (order.procType) {
    ...
    case OrderProcessType.auto:
      {
        /// 进入发货流程
        await executeChild(shipOrderWorkflow, { args: [orderSn], workflowId: `ship-${orderSn}` });
        /// 进入订单完成流程
        await executeChild(completeOrderWorkflow, { args: [orderSn], workflowId: `complete-${orderSn}` });
        console.log(`订单流程已结束!`);
      }
      break;
    // 手动发货订单
    case OrderProcessType.manual:
      {
        // 进入准备发货流程
        await executeChild(preparingOrderWorkflow, { args: [orderSn], workflowId: `preparing-${orderSn}` });
        // 进入等待收货流程
        await executeChild(receivingOrderWorkflow, { args: [orderSn], workflowId: `receiving-${orderSn}` });
        // 进入订单完成流程
        await executeChild(completeOrderWorkflow, { args: [orderSn], workflowId: `complete-${orderSn}` });
        console.log(`订单流程已结束!`);
      }
...
/** 检查订单支付状态流程 */
export const waitPaymentWorkflow = async (
  orderSn: string
): Promise<{ success: boolean; shouldClose: boolean }> => {
  console.log(`订单等待支付流程开始`);
  ...
  while (true) {
    try {
      ...
      if (order.payTime) {
        // 已支付
        console.log(`订单已支付, 准备进入下一个流程。`);
        return {
          success: true,
          shouldClose: false,
        };
      } else if (Date.now() - createTime >= timeoutDuration) {
        console.log(`订单支付超时, 订单将自动关闭`);
        // 超时未支付
        return {
          success: false,
          shouldClose: true,
