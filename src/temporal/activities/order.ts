import { MidwayContainer, getCurrentAsyncContextManager } from "@midwayjs/core";
...
export async function findOrder(orderSn: string) {
  const service = new MidwayContainer().get<OrderService>(OrderService);
  const order = await service.findBySn(orderSn);
  return order;
}

/** 查询订单支付情况 */
export async function getOrderPaidResult(orderSn: string): Promise<Pick<Order, 'payTime' | 'paySn' | 'paymentCode'>> {
  const order = await Order.findOne({
    where: { orderSn: orderSn },
    attributes: ['payTime', 'paySn', 'paymentCode'],
  });
  if (!order) {
